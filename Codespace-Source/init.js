...
if (!fs.existsSync('./build.config.nxlf')) {
    ...
    fs.writeFileSync('./build.config.nxlf', './config.nxconf', 'utf-8', (err) => {
        if (err) {
            console.error(err);
            return;
        }
    ...
    if (fs.existsSync('./build.config.nxlf')) {
        const checkcon = fs.readFileSync('build.config.nxlf', 'utf8');
    ...
    Initializing NextLanguage:
        ...
        Loaded Configuration File: ${checkcon}
        ...
        Copying over Important Directory Files:

            Detected Directory: /root/
            Checking if package.json exists!
            Package.json does not exists!

            Copied over File: package.json
            Copied over to: /root/

        Copied over Important Directory Files
