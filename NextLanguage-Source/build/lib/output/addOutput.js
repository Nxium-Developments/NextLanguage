...
module.exports = function addOutput(text) {
    ...
    if (fs.existsSync(path.join(__dirname, '../../log/') + currenttime + '-NXL.log')) {
        fs.appendFileSync(path.join(__dirname,'../../log/') + currenttime + '-NXL.log', text + '\n', 'utf8', (err) => {
            if (err) {
                console.error(err);
                return;
            }
        ...
        fs.writeFileSync(path.join(__dirname,'../../log/') + currenttime + '-NXL.log', `
